...
def load_cities(file_name):
    with open(file_name) as f:
        ...
        for city in file:
            ...
            cities[city['name']]['full_name'] = city['full_name']
            lng_lst = []
            lat_lst = []
            ...
            lng_lst.sort()
            lat_lst.sort()
            cities[city['name']]['bounding_box'] = [lng_lst[0], lat_lst[0], lng_lst[-1], lat_lst[-1]]
        return cities
...
def save_tweet(tweet, database):
    if str(tweet.id) not in database:
        id, rev = database.save({"_id": str(tweet.id),
                                 ...
                                 "city_full_name": city_info["full_name"],
                                 "city_coordinates": city_info["bounding_box"]})
...
def load_search_times(file_name, city):
    ...
    try:
        ...
        end_time, start_time = datetime.now(), (datetime.now() - timedelta(days=30))
...
if __name__ == '__main__':
    ...
    server = couchdb.Server('http://admin:admin@172.26.131.52:5984//')
    ...
    while True:
        for city, city_info in cities.items():
            query = "lang:en place:" + city_info['id']
            # load search time from "search_start_times.csv"
            start_time, end_time = load_search_times("search_start_times.csv", city)
            ...
            update_search_times("search_start_times.csv", city, start_time)
            print(city, "tweets harvest finished from", start_time, "to", end_time)

        if break_loop:
            break

    print("Tweet Searching end.")